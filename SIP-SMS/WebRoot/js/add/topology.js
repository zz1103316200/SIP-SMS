         ...
         if (!dragged(d)) {
         	...
         	if(d.value == "6"){
				...
				$("#bar-chart").empty();
				 $("#area-graph").empty();
                 $("#bar-chart").empty();
                 $.post("servicetopo",{serviceid:serviceid},function(text){
     				//alert("sdsd");
     				//寰楀埌鍥惧舰BarChar
     				showBarChar2(text.SingleServiceCallQuantity);
     				//鑾峰緱AreaGraph鐨刣ata锛屽苟寰楀埌鍥惧舰
     				var data_area = getAreaGraph2(text.SingleServiceRunTime,text.ordTime);
